...
from engine.workspace import CrossFilesystemApplyError, WorkspaceSession, apply_atomic
...
def test_audit_only_never_auto_upgrades(tmp_path: Path) -> None:
    ...
    assert result.gate_result.apply_authorized is False
...
def test_cross_volume_publish_fails(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(CrossFilesystemApplyError):
        apply_atomic(session, _gate())
